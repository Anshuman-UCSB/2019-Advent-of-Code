...
def expandDown():
    #print(grid)
    #print("size is going to be ", len(grid[0]))
    temp = []
    for i in range(len(grid[0])):
        temp.append(".")
    #print("temp: ", temp)
...
def expand(dir, size = 1):
    for i in range(size):
        if dir == 0:
            ...
            raise Exception("Invaild direction given")
...
def move(turn, face): #assuming old square is painted over
    facing = directions.index(face)
    "Facing is ^:1  >:2  v:3  <:4"
    dir = facing
    ...
    grid[loc[1]][loc[0]] += directions[dir]
    padSides()


def iterate(instructions):
    # instructions = [paint, direction]

    face = grid[loc[1]][loc[0]][1]
    ...
    move(instructions[1], face)


def addInstruction(instruct):
    ...
    for row in grid:
        for val in row:
            if len(val) > 1:
                ...
                print(val, end = "")
        print(" " * 50)
    print("Bot is at ({}, {})                           ".format(loc[0],loc[1]))
    print("                           ")
...
while not prc.finished:
    ...
    #print(cursor(0,2),prc)
    if(i%skipFrame == 0):
        print(cursor(5 ,5))
        printGrid()
        print("Visited {} square so far                      ".format(len(visited)))
        #print("\n"*15)
        #time.sleep(.2)

    if len(prc.outputs) >= 2:
        nextInp = []
        nextInp.append(prc.outputs.pop(0))
        nextInp.append(prc.outputs.pop(0))

        iterate(nextInp)

        prc.inputs.append(1 if grid[loc[1]][loc[0]][0] == "#" else 0)
